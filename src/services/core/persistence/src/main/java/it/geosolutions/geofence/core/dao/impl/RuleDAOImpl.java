    ...
    public long persist(Rule entity, InsertPosition position) {
        ...
        case FROM_START:
            ...
            if (list.isEmpty()) { // no rule found at given position: let's find out why
                ...
                if (LOGGER.isDebugEnabled())
                    LOGGER.debug("No rule found at position " + entity.getPriority()
                            + " -- rules count:" + count);
        ...
        case FROM_END:
            ...
            if (count == 0) {
                ...
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("Inserting rule -- count:" + count + " pos:-" + posFromEnd + "+"
                            + posFromStart);
                }

                if (posFromStart < 0) {
                    throw new IllegalArgumentException("Bad position from end (" + posFromEnd
                            + ") with count=" + count);
                }
                ...
                if (list1.isEmpty()) { // no rule found at given position: let's find out why
                    ...
                    int i = shift(basepri, 1);
                    if (LOGGER.isDebugEnabled()) {
                        LOGGER.debug("shifted " + i + " rules from priority " + basepri
                                + " downward");
                    }
                    entity.setPriority(basepri);
                }
    ...
    public Rule merge(Rule entity) {
        ...
        case 1:
            ...
            if (!existent.get(0).getId().equals(entity.getId())) {
                throw new DuplicateKeyException("Duplicating Rule " + existent.get(0) + " with "
                        + entity);
            }
            break;
