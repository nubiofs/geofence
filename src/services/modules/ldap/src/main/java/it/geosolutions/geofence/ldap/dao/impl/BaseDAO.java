 ...
 * 
 * It uses a spring-ldap LdapTemplate to communicate with the LDAP server. Currently only read type operations are supported (findAll, find, search).
 * 
 ...
 * 
 */
public abstract class BaseDAO<T extends RestrictedGenericDAO<R>, R> implements
        RestrictedGenericDAO<R> {
    ...
    /**
     * Search the given user id on the backup DAO, if defined. The id can be a classic id (>0) or an extId (<0). It's an extId if it's the id read
    ...
    /**
     * Search using the given filter on the LDAP server. Each result object is mapped with the given mapper. Given base and filter are used.
     * 
     * @param base
     * @param filter
     * @param mapper
