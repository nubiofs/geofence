...
public class RESTRuleServiceImpl
    ...
    public Response insert(RESTInputRule inputRule) throws NotFoundRestEx, InternalErrorRestEx {
    ...
    public void update(Long id, RESTInputRule rule) throws BadRequestRestEx, NotFoundRestEx, InternalErrorRestEx {
        ...
        try {
            ...
            if(rule.getRequest() != null) // checkme: can't be set to null?
                old.setRequest(rule.getRequest().isEmpty()? null: rule.getRequest());

            if(rule.getWorkspace() != null) // checkme: can't be set to null?
                old.setWorkspace(rule.getWorkspace().isEmpty()? null : rule.getWorkspace());

            if(rule.getLayer() != null) // checkme: can't be set to null?
            ...
            if(rule.getConstraints() != null) {
                ...
                LayerDetails detailsOld = old.getLayerDetails();

                if(constraintsNew.getAllowedStyles() != null) {
                    detailsOld.getAllowedStyles().removeAll(constraintsNew.getAllowedStyles());
                    detailsOld.getAllowedStyles().addAll(constraintsNew.getAllowedStyles());
                }
                ...
                old.setLayerDetails(detailsOld); // checkme!
            }
